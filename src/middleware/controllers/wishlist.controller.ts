...
export async function UserModifyRestricted(req, res, next) {
	try {
		const checkUserID = await wishlistService.modifyAuth(
			req.params.id,
...
export async function create(req, res) {
	const resGen = new responseGenerator(res, "wishlist", messageTypes.CREATE);
...
export async function pages(req, res) {
	...
	try {
		const data = await wishlistService.getPages(page - 1, items_per_page);
...
export async function getOne(req, res) {
	...
	try {
		const data = await wishlistService.getOne(req.params.id);
		return resGen.data(responseTypes.SUCCESS, data);
	} catch (e) {
		return resGen.error(responseTypes.SERVER_ERROR, e.message);
	}
...
export async function deleteOne(req, res) {
	const resGen = new responseGenerator(res, "wishlist", messageTypes.DELETE);
	try {
		await wishlistService.deleteOne(req.params.id);
